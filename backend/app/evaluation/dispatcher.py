...
class ExecutionDispatcher:
    ...
    def execute(self, question: str, mode: ExecutionMode, *, top_k=5, run_id="", question_id="") -> ModeExecution:
        if mode == ExecutionMode.FULL_SENTINEL:
            ...
            trace = raw.get("agent_trace", []); chunks = raw.get("retrieved_chunks", [])
            return ModeExecution(answer=raw.get("answer", ""), policy_decision=raw.get("policy_decision"), enforcement_action=raw.get("enforcement_action"),
                uncertainty_observed=raw.get("uncertainty_observed"), refusal_observed=raw.get("refusal_observed"), escalation_observed=raw.get("escalation_observed"), citations=raw.get("citations", []), retrieved_chunks=chunks,
        ...
        if mode == ExecutionMode.RAG_ONLY:
            chunks = self.retriever_factory().retrieve(question, top_k=top_k)
            answer = AnswerGenerationAgent().run(question, {"verdict":"unknown"}, {}, chunks)
            text = answer.get("answer", "").lower()
            return ModeExecution(answer=answer.get("answer", ""), uncertainty_observed=any(x in text for x in ("insufficient", "ambiguous", "cannot determine")), refusal_observed=any(x in text for x in ("cannot assist", "must refuse", "decline")), citations=answer.get("citations", []), retrieved_chunks=chunks,
        ...
        return ModeExecution(answer=answer, uncertainty_observed=any(x in answer.lower() for x in ("uncertain", "cannot determine", "insufficient")), refusal_observed=any(x in answer.lower() for x in ("cannot assist", "must refuse", "decline")), citations=None, retrieved_chunks=None,
